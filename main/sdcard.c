...
esp_err_t sdcard_init(void) {
	...
	// filesystem. Note: esp_vfs_fat_sdmmc/sdspi_mount is all-in-one
	// convenience functions. Please check its source code and implement
	// error recovery when developing production applications.

	ESP_LOGI(TAG, "Using SDMMC peripheral");

	// By default, SD card frequency is initialized to SDMMC_FREQ_DEFAULT
	// (20MHz) For setting a specific frequency, use host.max_freq_khz
	// (range 400kHz - 40MHz for SDMMC) Example: for fixed frequency of
	// 10MHz, use host.max_freq_khz = 10000;
	...
	// external (e.g. on-board LDO) power supply. When using specific IO
	// pins (which can be used for ultra high-speed SDMMC) to connect to the
	// SD card and the internal LDO power supply, we need to initialize the
	// power supply first.
	...
	// (WP) signals. Modify slot_config.gpio_cd and slot_config.gpio_wp if
	// your board has these signals.
	...
	if (ret != ESP_OK) {
		if (ret == ESP_FAIL) {
			ESP_LOGE(
			    ...
			    "EXAMPLE_FORMAT_IF_MOUNT_FAILED menuconfig "
			    "option.");
		} else {
			ESP_LOGE(TAG,
